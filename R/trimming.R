...
check.arguments = function(ldf = parent.frame()$ldf,
  ...
  for(i in names(groupers)){
    if(value.col %in% groupers[[i]]) return("The value-col cannot be defined in '",i,"'")
    ...
    if(sum(is.na(ldf[,groupers[[i]]])) > 0) return("The identifier-columns defined by '",i,"' can not contain NA")
  ...
  for(i in names(selectors)){
    if(!is.null(selectors[[i]])){
      if(!is.list(selectors[[i]])) return("'",i,"' must be a list")
      if(any(is.null(names(selectors[[i]])))) return("'",i,"' must have element names corresponding to column names of 'ldf'")
      if(any(!names(selectors[[i]]) %in% colnames(ldf))) return(paste0("[",paste(names(apply.to)[!names(apply.to) %in% colnames(ldf)], collapse = ','), "] required by '",i,"', but not present in the colnames of 'ldf'"))
...
#' @param stat.func Name of a function that can take a vector and output a scalar. This function is used to summarise the values of each grouping. Defaults to 'mean'. 'na.rm = T' is passed as additional argument.
#' @param criterion Function that defines in which cases an output of the 'stat.func' fulfills the exclusion criterion. Defaults to 'function(stat) return(stat <= -.5)', i.e. exclusion-worthy are groupings whose summary value is smaller than 0.5 The function must use the arg 'stat', which is a scalar.
#' @param return.stats Returns the ldf and trimming statistics in a list. FALSE returns just the ldf. Defaults to FALSE.
#' @param return.format 'ldf' for long dataframe (as provided) or 'wdf' for wider dataframe, where the values of each measure are put in a separate column. Defaults to 'ldf'.
#' @param notify Provides progress information. Defaults to TRUE.
#' @importFrom dplyr '%>%'
#' @keywords exclusion
#' @export
...
exclude.groupings = function(ldf, group.by, exclude.by = NULL, test = NULL, test.not = NULL, excl = NULL, excl.not = NULL, stat.func = 'mean', criterion = function(S) return(S <= 0.5), value.col = 'value', report.by = NULL, return.report = F, return.stats = F, notify = T){
  ...
  if(length(formals(criterion)) > 1) stop("THe function defined as 'criterion' can only take a single argument (the statistic calculated by 'stat.func')")
  ...
  # Apply the 'stat.func' to the values of the groupings
  stat = suppressMessages({ldf2 %>% dplyr::summarise(grouping.statistic = do.call(match.fun(stat.func), list(!!dplyr::sym(value.col), na.rm = T)), .groups = 'keep')})
  ...
  colnames(stat)[colnames(stat) %in% 'grouping.statistic'] = stat.func
...
trim.by_interval = function(ldf, group.by, apply.to = NULL, apply.not.to = NULL, trim = 0.025, trim.ties = T, warn.ties = T, view.ties = F, ignore.na = T, report.by = NULL, return.report = F, notify = T, value.col = 'value'){
...
trim.by_spread = function(ldf, group.by, apply.to = NULL, apply.not.to = NULL, center.func = 'mean', spread.func = 'sd', trim = 2.5, report.by = NULL, return.report = F, return.stats = F, notify = T, value.col = 'value'){
...
#' @param rm.legend Removes the legend. Defaults to FALSE.
#' @importFrom dplyr '%>%'
...
view.data = function(ldf, group.by, view = NULL, view.not = NULL, view.together = NULL, view.separate = NULL, observation = NULL, by.observation = F, sort.observation = F, type = 'point_line', value.col = 'value', add.names = NULL, group.sep = '.', label = '', fill = F, bins = 30, rm.legend = F){
  ...
  # Collapse identifiers in groupinc cols to be viewed together into a single col
  ...
  for (i in 1:length(ldf2.list)) {
    ...
    if(!by.observation){
      ...
      ldf2.list[[i]]$single_grouping.column = factor(ldf2.list[[i]]$single_grouping.column, levels = unique(ldf2.list[[i]]$single_grouping.column))
      # if(any(type %in% c('box','violin','hist','density'))) ldf2.list[[i]]$single_grouping.column = as.character(ldf2.list[[i]]$single_grouping.column)

      # Show observation on x-axis, color by single_grouping.column
      if(any(type %in% c('point','point_line'))) g = g + ggplot2::geom_point(data = ldf2.list[[i]], ggplot2::aes(x = grouping_observation.column, y = !!dplyr::sym(value.col), color = single_grouping.column), show.legend = if(rm.legend) F else T)
      if(any(type %in% c('line','point_line')))  g = g + ggplot2::geom_line( data = ldf2.list[[i]], ggplot2::aes(x = grouping_observation.column, y = !!dplyr::sym(value.col), color = single_grouping.column), show.legend = if(rm.legend) F else T)

      # Show single_grouping.column on x-axis, color by single_grouping.column
      if(any(type == 'box'))    g = g + ggplot2::geom_boxplot(data = as.data.frame(ldf2.list[[i]]), ggplot2::aes(x = single_grouping.column, y = !!dplyr::sym(value.col), color = single_grouping.column, fill = if(fill) single_grouping.column), show.legend = if(rm.legend) F else T)
      if(any(type == 'violin')) g = g + ggplot2::geom_violin( data = as.data.frame(ldf2.list[[i]]), ggplot2::aes(x = single_grouping.column, y = !!dplyr::sym(value.col), color = single_grouping.column, fill = if(fill) single_grouping.column), show.legend = if(rm.legend) F else T)

      # Show measure on x-axis, color by single_grouping.column
      if(type == 'hist')      g = g + ggplot2::geom_histogram(data = ldf2.list[[i]], ggplot2::aes(x = !!dplyr::sym(value.col), color = single_grouping.column, fill = if(fill) single_grouping.column), bins = bins, show.legend = if(rm.legend) F else T)
      if(type == 'density')   g = g + ggplot2::geom_density(  data = ldf2.list[[i]], ggplot2::aes(x = !!dplyr::sym(value.col), color = single_grouping.column, fill = if(fill) single_grouping.column), alpha = 0.1, show.legend = if(rm.legend) F else T)
    }else{
      # All plots except point and line plots need levels in the grouping (here grouping_observation.column) to plot properly
      ldf2.list[[i]]$grouping_observation.column = factor(ldf2.list[[i]]$grouping_observation.column, levels = unique(ldf2.list[[i]]$grouping_observation.column))
      # if(any(type %in% c('box','violin','hist','density'))) ldf2.list[[i]]$grouping_observation.column = as.character(ldf2.list[[i]]$grouping_observation.column)

      # Show single_grouping.column on x-axis, color by observation
      if(any(type %in% c('point','point_line'))) g = g + ggplot2::geom_point(data = ldf2.list[[i]], ggplot2::aes(x = single_grouping.column, y = !!dplyr::sym(value.col), group = grouping_observation.column, color = as.factor(grouping_observation.column)), show.legend = if(rm.legend) F else T)
      if(any(type %in% c('line','point_line'))) g = g + ggplot2::geom_line(data = ldf2.list[[i]], ggplot2::aes(x = single_grouping.column, y = !!dplyr::sym(value.col), group = grouping_observation.column, color = as.factor(grouping_observation.column)), show.legend = if(rm.legend) F else T)

      # Show observation on x-axis, color by observation
      if(any(type == 'box'))    g = g + ggplot2::geom_boxplot(data = as.data.frame(ldf2.list[[i]]), ggplot2::aes(x = grouping_observation.column, y = !!dplyr::sym(value.col), color = grouping_observation.column, fill = if(fill) grouping_observation.column), show.legend = if(rm.legend) F else T)
      if(any(type == 'violin')) g = g + ggplot2::geom_violin( data = as.data.frame(ldf2.list[[i]]), ggplot2::aes(x = grouping_observation.column, y = !!dplyr::sym(value.col), color = grouping_observation.column, fill = if(fill) grouping_observation.column), show.legend = if(rm.legend) F else T)

      # Show measure on x-axis, color by observation
      if(type == 'hist')      g = g + ggplot2::geom_histogram(data = ldf2.list[[i]], ggplot2::aes(x = !!dplyr::sym(value.col), color = grouping_observation.column, fill = if(fill) grouping_observation.column), bins = bins, show.legend = if(rm.legend) F else T)
      if(type == 'density')   g = g + ggplot2::geom_density(  data = ldf2.list[[i]], ggplot2::aes(x = !!dplyr::sym(value.col), color = grouping_observation.column, fill = if(fill) grouping_observation.column), alpha = 0.1, show.legend = if(rm.legend) F else T)}
    ...
    if(fill) g = g + ggplot2::guides(fill = guide_legend(override.aes = list(color = NULL)))
    if(rm.legend) g = g + ggplot2::theme(legend.position = 'none')
